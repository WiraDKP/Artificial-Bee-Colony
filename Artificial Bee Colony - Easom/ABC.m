function [bestbee, mincost] = ABC(problem, nvar, bound, param)
    ...
    for iter = 1:itermax
        [X,Y] = meshgrid(xmin:0.05:xmax, xmin:0.05:xmax);
        contour(X,Y,-cos(X).*cos(Y).*exp(-((X-pi).^2+(Y-pi).^2)), 25); hold on;
        scatter(pi,pi,35,'ok','filled');
